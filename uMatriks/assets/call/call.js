"use strict";
/**
 * This is based on matrix internal module from matrix-js-sdk.
 * Modified heavily to work with oxide
 */
var DEBUG = false;  // set true to enable console logging.
var ubuntuTurnServers;
...
oxide.addMessageHandler("placeVoiceCall", function (msg) {
...
oxide.addMessageHandler("placeVideoCall", function (msg) {
    mxCall.localVideoElement = document.getElementById("self");
    mxCall.remoteVideoElement = document.getElementById("remote");;
...
oxide.addMessageHandler("setCallId", function (msg) {
    mxCall.callId = msg.args.callId;
    msg.reply({str: "OK"});
});

oxide.addMessageHandler("setTurnServers", function (msg) {
    // map the response to a format that can be fed to
    // RTCPeerConnection
    const servers = {
        urls: msg.args.uris,
        username: msg.args.username,
        credential: msg.args.password
    };
    ubuntuTurnServers = [servers];
    msg.reply({str: "OK"});
})

oxide.addMessageHandler("initWithInvite", function (msg) {
    console.log("initWithInvite");
    mxCall.callId = msg.args["call_id"]
    mxCall.localVideoElement = document.getElementById("self");
  	mxCall.remoteVideoElement = document.getElementById("remote");;
      mxCall._initWithInvite(msg.args);
    msg.reply({str: "OK"});
});

oxide.addMessageHandler("gotRemoteCandidate", function (msg) {
    mxCall._gotRemoteIceCandidate(msg.args.candidate);
    msg.reply({str: "OK"});
});


oxide.addMessageHandler("gotRemoteCandidates", function (msg) {
    console.log("gotRemoteCandidates", msg.args);
    console.log(msg.args.candidates.length);
    for (var i = 0; i < msg.args.candidates.length; i++) {
      console.log(msg.args.candidates[i].sdpMid)
  		mxCall._gotRemoteIceCandidate(msg.args.candidates[i]);
  	}
    msg.reply({str: "OK"});
});

oxide.addMessageHandler("answer", function (msg) {
    mxCall.answer();
    msg.reply({str: "OK"});
});

oxide.addMessageHandler("receivedAnswer", function (msg) {
    mxCall._receivedAnswer(msg.args);
    msg.reply({str: "OK"});
});

oxide.addMessageHandler("onHangupReceived", function (msg) {
    mxCall._onHangupReceived(msg.args);
    msg.reply({str: "OK"});
});

oxide.addMessageHandler("hangup", function (msg) {
    mxCall.hangup("", false);
    msg.reply({str: "OK"});
});

oxide.addMessageHandler("onAnsweredElsewhere", function (msg) {
    mxCall._onAnsweredElsewhere();
    msg.reply({str: "OK"});
});

oxide.addMessageHandler("init", function (msg) {
    // Init MX call
    console.log("oxide" + msg.args.roomId + msg.args.callId)
    mxCall = createNewMatrixCall(msg.args.roomId);
    mxCall.callId = msg.args.callId;
    msg.reply({str: "OK"});
});
...
function AndroidSendEvent(roomId, event, content) {
    console.log(roomId, event, content);
    oxide.sendMessage("event", {roomId: roomId, event: event, content: content})
}
...
function MatrixCall(opts) {
    ...
    console.log(this.turnServers[0]["urls"][0])
}
...
MatrixCall.prototype._create_answer = function() {
    ...
    self.peerConn.createAnswer(function(description) {
      ...
      debuglog("Create Answer FAILED!! " + e)
    });
...
MatrixCall.prototype._gotRemoteIceCandidate = function(cand) {
    ...
    this.peerConn.addIceCandidate(
        ...
        function() { debuglog("ICE ADDED"); },
        function(e) { debuglog("FAILED TO ADD ICE"); }
    );
};
...
MatrixCall.prototype._receivedAnswer = function(msg) {
    ...
    debuglog(msg.answer.sdp);
    debuglog(msg.answer.type);
...
MatrixCall.prototype._getLocalOfferFailed = function(error) {
    AndroidCallError("_getLocalOfferFailed");
...
function createNewMatrixCall(/*client,*/ roomId) {
    ...
    var turnServs = null;
    if (ubuntuTurnServers) {
        turnServs = ubuntuTurnServers;
    }
